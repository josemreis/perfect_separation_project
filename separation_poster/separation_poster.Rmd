...
3.  From each population draw $\frac{1.000.000}{n_{sample}}$ samples, where $n_{sample} = \{25, 50, 75\}$. For each sample check whether perfect separation or quasi-perfect separation occurred [@detect]
...
We ended up with 55 simulated populations each with a different parameter and 1000 simulated samples (N = 55000) which we used to study the properties of the estimation methods.
...
data %>% 
  ...
  arrange(abs(`Bias averaged over the 55 populations (#samples = 1000)`)) %>%
...
data %>% # same but more straight forward
  ...
  labs(caption = "Error bars are computed |nusing the monte carlo standard error of the 1000 simulations for each of the 55 beta values.",
...
* PML and assigning prior distributions to the parameter curb this problem
